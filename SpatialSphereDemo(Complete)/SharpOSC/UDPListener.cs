...
namespace SharpOSC
{
    ...
    public class UDPListener : IDisposable
	{
        ...
        {
            await receivingUdpClient.ConnectAsync(remoteHostName, remoteServiceNameOrPort);
        }
        ...
        public UDPListener(int port)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				try
				{
					receivingUdpClient = new DatagramSocket();
                    Connect(new HostName("127.0.0.1"), "12345");
                    break;
				}
				catch (Exception)
				{
		...
		void ReceiveCallback(IAsyncResult result)
		{
			...
			try
			{
                bytes = null;// receivingUdpClient.EndReceive(result, ref RemoteIpEndPoint);
			}
